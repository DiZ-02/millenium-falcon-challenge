...
def check_dependencies(ctx: Context) -> None:
    ...
    ctx.run(
        safety.check(requirements, ignore_vulns={"70612": {"reason": "disputed", "expires": False}}),
...
def check_api(ctx: Context) -> None:
    ...
    Parameters:
        ctx: The context instance (passed automatically).
    """
    from griffe import check as g_check

    griffe_check = lazy(g_check, name="griffe.check")
    ctx.run(
        griffe_check("falcon", search_paths=["src"], color=True),
        title="Checking for API breaking changes",
        command="griffe check -s src falcon",
        nofail=True,
    )
